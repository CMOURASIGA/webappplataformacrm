...
export function AppLayout() {
  ...
  if (!isInitialized) return <div className="flex h-screen items-center justify-center bg-slate-50"><LoadingState label="Carregando..." /></div>;

  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#F1F5F9] font-sans text-slate-900">
      <Sidebar collapsed={false} mobileOpen={mobileMenuOpen} onMobileClose={() => setMobileMenuOpen(false)} isMaster={isMaster} activeTenantId={activeTenantId} tenant={tenant} primaryColor={primaryColor} currentUser={currentUser} onLogout={handleLogout} />
      <main className="flex min-w-0 flex-1 flex-col">
        <Header breadcrumb={getBreadcrumb(location.pathname)} isMaster={isMaster} tenants={tenants} activeTenantId={activeTenantId} onChangeActiveTenant={setActiveTenantId} currentUser={currentUser} onOpenMobileMenu={() => setMobileMenuOpen(true)} sidebarCollapsed={false} onToggleSidebar={() => undefined} onLogout={handleLogout} />
